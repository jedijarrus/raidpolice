# Raidpolice — Finaler Umsetzungsplan: Boss-Knowledge-Base + Diagnose-Features

## Vorbemerkung: ID-Abgleich Recherche vs. eigene Logs

Die eigenen Logs nutzen teils **andere IDs als die Recherche** (WCL loggt oft die Effekt-/Trigger-ID, nicht die Cast-ID). Beispiele: Blast Nova `30613` (Logs) vs. `30616` (Recherche), Whirlwind Leotheras `37641` vs. `37640`, Pounding `34164` vs. `34162`, Flame Patch `35383` vs. `35380`, Arcane Missiles `39414` vs. `33031`. Die KB führt deshalb pro Mechanik `spellIds` (in eigenen Logs beobachtet = Match-Priorität) und `altIds` (Recherche-verifizierte Varianten). Matching läuft immer über die Vereinigungsmenge. Wichtiger Befund: `36971` (Logs, „Fireball“, 40 Tode bei Kael) ist **Capernians** Fireball — Advisor-Abilities müssen unter dem Kael-Encounter gemappt sein. Außerdem: 662/1579 Tode sind „Melee“ (`1`, Hydross `-16`) — dafür braucht es einen kontextbasierten Klassifikator statt Spell-Matching (Feature 2).

---

## 1) Boss-Knowledge-Base (JS)

```js
// kb/bossKB.js — Quellen: Wowhead-TBC / CMaNGOS / Icy Veins (Agenten-Recherche 2026-06) + eigene WCL-Logs (41 Wipe-Reports, 1579 Tode)
// idSource: 'logs' = in eigenen killingBlows beobachtet, 'research' = extern verifiziert, 'both'
...
const bossKB = {
  ...
  'Morogrim Tidewalker': {
    ...
    mechanics: [
      ...
      { key: 'globule', name: 'Water Globule', spellIds: [37871], idSource: 'research', avoidable: true, blame: [BLAME.PLAYER, BLAME.ASSIGNMENT],
  ...
  'Lady Vashj': {
    ...
    mechanics: [
      ...
      { key: 'shoot', name: 'Shoot/Multi-Shot (Entangle-Fenster)', spellIds: [38295], altIds: [38310,38316], idSource: 'both', avoidable: false, blame: [BLAME.HEALER],
  ...
  "Kael'thas Sunstrider": {
    ...
    mechanics: [
      ...
      { key: 'psychicBlow', name: 'Psychic Blow (Thaladred)', spellIds: [36966], altIds: [36965,30225], idSource: 'research', avoidable: true, blame: [BLAME.PLAYER],
      ...
      { key: 'phoenixBurn', name: 'Phoenix Burn', spellIds: [36720,36721], altIds: [36723], idSource: 'research', avoidable: true, blame: [BLAME.ASSIGNMENT],
  ...
  'High King Maulgar': {
    ...
    mechanics: [
      ...
      { key: 'arcaneExplosion', name: 'Arcane Explosion (Kiggler)', spellIds: [33237], altIds: [33175,33173], idSource: 'research', avoidable: true, blame: [BLAME.ASSIGNMENT],
        diagnosis: 'Kiggler muss auf Range getankt werden.' },
      { key: 'deathCoil', name: 'Death Coil / Dark Decay (Olm)', spellIds: [33130,33129], altIds: [33131], idSource: 'research', avoidable: false, blame: [BLAME.ASSIGNMENT, BLAME.HEALER],
        diagnosis: 'Warlock-Tank-Setup / Felhunter-Banish prüfen.' },
    ],
    mustInterrupt: [{ name: 'Prayer of Healing (Blindeye)', spellIds: [33152], altIds: [33144,33147] }],
  },

  'Gruul the Dragonkiller': {
    zone: "Gruul's Lair",
    mechanics: [
      { key: 'shatter', name: 'Shatter', spellIds: [33671], altIds: [33654,33525,33652,33572], idSource: 'both', avoidable: true, blame: [BLAME.PLAYER],
  ...
  'Magtheridon': {
    ...
    mechanics: [
      ...
      { key: 'p1Channelers', name: 'P1: Shadow Bolt Volley / Fear', spellIds: [30510,30530], altIds: [39175,30528,30511,30531], idSource: 'research', avoidable: true, blame: [BLAME.INTERRUPT, BLAME.ASSIGNMENT],
  ...
  'Shade of Aran': {
    ...
    mechanics: [
      ...
      { key: 'arcaneCombo', name: 'Arcane Explosion (Teleport-Kombo)', spellIds: [29973], altIds: [30035,29991], idSource: 'research', avoidable: true, blame: [BLAME.PLAYER, BLAME.DISPEL],
      ...
      { key: 'pyroblast', name: 'Pyroblast (nach Drink)', spellIds: [29978], altIds: [29963,30024], idSource: 'research', avoidable: false, blame: [BLAME.HEALER],
      ...
      { key: 'nukes', name: 'Frostbolt/Fireball/Missiles', spellIds: [29954,29953,29955,29964], idSource: 'research', avoidable: false, blame: [BLAME.INTERRUPT, BLAME.HEALER],
  ...
  'Prince Malchezaar': {
    ...
    mechanics: [
      ...
      { key: 'hellfire', name: 'Infernal Hellfire', spellIds: [30859], altIds: [39131,30834,37277], idSource: 'research', avoidable: true, blame: [BLAME.PLAYER, BLAME.RAIDLEAD],
      ...
      { key: 'thrash', name: 'Thrash (P2)', spellIds: [12787], altIds: [30901], idSource: 'research', avoidable: false, blame: [BLAME.HEALER, BLAME.TANK],
...
**Bewusst weggelassen** (Recherche unsicher): Globule-Explosions-ID, Strider-Panic-ID, Tainted-Core-Root, Aran-Teleport-Sub-ID, Solarian-Transform-Details, Wasserelementar-Summon-ID (die `37674` der Kara-Recherche kollidiert mit Leotheras' Chaos Blast — gestrichen).

---

## 2) Priorisierte Feature-Liste (max. 8)

Rate-Limit-Rahmen: Free-Tier 3600 P/h, ein kompletter Abend-Report kostet ~40-60 P → **Quota ist kein Engpass**, solange abgeschlossene Fights (wie bisher via `settings_hash`) dauerhaft gecacht und `rateLimitData` mitgeloggt wird. Features 1-4 brauchen **null neue API-Calls**.

| # | Feature | Was | Daten | Aufwand | Raidleiter-Nutzen |
|---|---------|-----|-------|---------|-------------------|
| 1 | **Tod-Diagnose-Engine** | `killingBlow.id` jedes Todes gegen `spellIndex` matchen → pro Tod: Mechanik, vermeidbar ja/nein, Blame-Kategorie, Diagnose-Satz in der bestehenden Wipe-Timeline | **Vorhanden**: `wipes.deathEntries[].killingBlow` (1579 Tode bereits im Cache) | **M** | Macht aus „X starb an Blast Nova“ sofort „Cube-Klick-Kette versagt — Backup-Rotation prüfen“ — die Schuldfrage wird objektiv statt gefühlt. |
| 2 | **Melee-Tod-Klassifikator** | Die 662 undiagnostizierten Melee-Tode (`1`/`-16`) per Kontext einordnen: Opfer-Rolle (`tankInfo`), Zeitnähe zu `phaseTransitions`/Add-Spawns (`curves.addHp`), Killer-`source`-NPC → Heuristik „Add-Pickup / Aggro / Tank-Spike“ + boss-spezifische `meleeDiagnosis` aus der KB | **Vorhanden**: `wipes`-JSON (deathEntries.source, phaseTransitions, addHp, tankInfo) | **M** | 42% aller Tode sind heute „Melee = keine Aussage“ — der Klassifikator verwandelt den größten blinden Fleck in konkrete Pickup-/Threat-/Heiler-Diagnosen. |
| 3 | **Wiederholungstäter-Aggregation** | Pro Spieler über alle Reports: vermeidbare Tode je Mechanik (aus Feature 1) + `avoidablePerPlayer`, als sortierte „Wer stirbt woran ständig?“-Tabelle mit Trend | **Vorhanden**: `report_analysis` (wipes, avoidable) quer über 41 Reports | **S** | Zeigt vor dem Raid, wem man welche EINE Mechanik nochmal erklären muss — gezieltes Coaching statt Gießkannen-Ansage. |
| 4 | **Boss-Progression-Trend** | `bossPercentage`/`fightPercentage` + `lastPhase`-Äquivalent (aus `phaseTransitions`) je Boss über alle 88 Reports als Kurve: lernt der Raid oder stagniert er? | **Vorhanden**: `fights_json` (heute nur ~4-6× referenziert — ungenutzt!) | **S** | „Pull 3: 46% → Pull 9: 12%“ über Wochen sichtbar — Entscheidungsgrundlage für Strat-Wechsel vs. Weiterwipen. |
| 5 | **Kill-Death-Coverage** | Tode in Kill-Fights nachladen (heute existieren Ability-Daten nur für Wipes): v2 `events(dataType: Deaths, fightIDs: [...])` pro Kill, durch dieselbe Diagnose-Engine schicken, im Cache persistieren | **Neu (WCL v2)**: Deaths-Events, ~1 P/Fight, abgeschlossene Fights → einmalig + unbegrenzt cachebar; Backfill 88 Reports ≈ 300-500 P, einmalig | **M** | Auch „dreckige Kills“ (5 Tote am Endboss) werden auswertbar — genau die Tode, die beim nächsten Progress-Boss zum Wipe werden. |
| 6 | **Pflicht-Interrupt/Dispel-Audit** | KB-Felder `mustInterrupt` (Healing Wave 38330, Dark Mending 30528, Great Heal 33387, Prayer of Healing 33152) und Dispel-Pflichten (SW:P, Vile Sludge, Searing Cinders, Chains of Ice) gegen Ist-Daten prüfen: wer war zugeteilt, was ging durch? | **Teils vorhanden** (`wipes.interrupts/dispels` aggregiert), **neu** für Detailtiefe/Kills: v2 `table(dataType: Interrupts/Dispels)`, ~2 P/Fight | **M** | Beantwortet die häufigste Nach-Wipe-Frage („wer hatte den Kick?“) mit Daten statt Voice-Chat-Erinnerung. |
| 7 | **Mechanik-Treffer-Prüfer (filterExpression)** | Boss-spezifische Checks serverseitig: Static-Charge-Umstehende, Shatter-Schaden >0 bei Spread-Pflicht, Hits durch alte Infernale, Beam-Buff-Klau (30421-30423-Auren auf falschen Spielern) — KB liefert die IDs, `filterExpression` filtert | **Neu (WCL v2)**: `events`/`table` mit `filterExpression`, ~1 P/Check, nur für ausgewählte Pulls on-demand | **L** | Erwischt die Fehler, die NICHT mit Tod enden (Beam geklaut, in Spores gestanden und überlebt) — Frühwarnung vor dem Wipe von morgen. |
| 8 | **Abend-Synthese mit Quota-Wächter** | Pro Raidabend ein generierter Report: Top-3-Wipe-Ursachen (aus 1+2), Wiederholungstäter (3), Progressionskurve (4), Kick/Dispel-Fails (6), inkl. `rateLimitData`-Anzeige + Low-Prio-Queue bei >80% Punkteverbrauch | Kombiniert 1-6; `rateLimitData` ≈ 0 P | **L** | Ersetzt die 45-Minuten-Log-Sichtung nach dem Raid durch einen 2-Minuten-Report mit den drei Dingen, die der Raid morgen anders machen muss. |

**Empfohlene Reihenfolge:** 1 → 2 → 3 → 4 (rein Cache-basiert, sofortiger Nutzen, kein API-Risiko), dann 5 (einmaliger Backfill), dann 6, dann 8; Feature 7 zuletzt als On-Demand-Werkzeug für Progression-Bosse (aktuell: Vashj/Kael — dort liegen mit Forked Lightning 27, Capernian-Fireball 40 und Bomb 31 Toden auch die größten offenen Baustellen der eigenen Logs, neben dem Magtheridon-Befund Blast Nova 151).